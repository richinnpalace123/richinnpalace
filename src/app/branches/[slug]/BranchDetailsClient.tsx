...
export default function BranchDetailsClient({ branch, allBranches }: BranchDetailsClientProps) {
  ...
  const [selectedRooms, setSelectedRooms] = useState<Record<string, number>>(() => {
    const firstRoom = branch.roomVarieties[0];
  ...
  const perNightRoomsTotal = useMemo(() => {
    return branch.roomVarieties.reduce((sum, room) => {
      const count = selectedRooms[room.id] || 0;
      return sum + room.price * count;
    }, 0);
  }, [branch.roomVarieties, selectedRooms]);
  ...
  // Build summary label of selected items
  const selectedRoomsSummaryList = useMemo(() => {
    return branch.roomVarieties
  ...
  return (
    <div className="bg-bg-dark min-h-screen pt-20 md:pt-28 pb-20 font-sans">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        {/* Top Two Column Layout: Main Details & Sticky Checkout Panel */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
          ...
          <div className="lg:col-span-8 space-y-8 md:space-y-12">
            ...
            <div className="space-y-2">
              <span className="text-[10px] md:text-xs uppercase tracking-[0.25em] text-gold font-medium block">
                {branch.area.toUpperCase()} · CHENNAI SANCTUARY
              </span>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-4">
                {branch.roomVarieties.map((room, index) => {
              ...
              <div className="p-4 sm:p-5 rounded-2xl bg-surface-dark/40 border border-border-dark/60 flex items-center justify-between">
                <div>
                  <span className="text-xs text-text-offwhite font-medium block">
                    Extra Bed Option (₹{branch.extraBedRate} / night)
                  </span>
            ...
            {/* Chamber Specifications (Matching Screenshot 1) */}
            <div className="space-y-4">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                {branch.inclusions.map((inclusion, idx) => (
                ...
                <div className="flex items-center space-x-3 text-text-gray/90 text-xs sm:text-sm font-sans font-light">
                  <span className="w-1.5 h-1.5 rounded-full bg-gold shrink-0" />
                  <span>Extra Bed Available (₹{branch.extraBedRate}/night)</span>
                </div>
              </div>
            </div>
          ...
          {/* RIGHT COLUMN: STICKY CHECKOUT PANEL (Matching Screenshot 1) */}
                ...
                <div className="flex items-baseline justify-between">
                  ...
                  <div className="text-right">
                    <span className="font-serif text-2xl md:text-3xl text-gold font-light">
                      {formatPrice(perNightGrandRate > 0 ? perNightGrandRate : branch.startingPrice)}
                    </span>
